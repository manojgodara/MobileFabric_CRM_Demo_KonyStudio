			...
			this.getToken = function (fromserver, successCallback, failureCallback) {
				...
				if (fromserver === true) {
					...
					_claimsRefresh(null,
						...
						konyRef.currentClaimToken = token.claims_token.value;
						kony.sdk.verifyAndCallClosure(successCallback, token.claims_token.value);
					},
						function (err) {
					...
					if (konyRef.tokens[_providerName]) {
						...
						if (_exp && _exp > (new Date().getTime())) {
							...
							konyRef.currentClaimToken = val.claims_token.value;
							kony.sdk.verifyAndCallClosure(successCallback, val.claims_token.value);
						} else {
							...
							_claimsRefresh(null,
								...
								konyRef.tokens[_providerName] = val.claims_token.value;
								logger.log("### AuthService::getToken extracting token successfull. calling success callback");
						...
						return kony.sdk.verifyAndCallClosure(failureCallback, null);
					}
				}
			};
			/**
...
kony.sdk.prototype.getIntegrationService = function (serviceName) {
	...
	if(this.integsvc != null){
		if(this.integsvc[serviceName] != null){
			kony.print("found integration service" + this.integsvc[serviceName]);
...
function ReportingService(konyRef) {
	...
	this.report = function (reportingGroupID, metrics) {
		...
		konyRef.getDataStore().setItem("konyCustomReportData",reportData);
